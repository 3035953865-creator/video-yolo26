 ...
 * @file main.cc
 * @brief 该程序在rv1126b上使用端到端yolo26n模型进行目标检测,
 * 		  并通过 OpenCV 进行摄像头视频流处理并显示结果。
 * @author luominghui
...
int main(int argc, char **argv)
{
    if (argc <= 2 || argc>3)
    {
        printf("%s <model_path> [camera_index]\n", argv[0]);
        return -1;
    }

    const char *model_path = argv[1];
    int camera_index = 31; // default value
    if (argc == 3) {
    ...
    while(true){
        ...
        if (frame_count % 10 == 0) { // 每10帧更新一次FPS显示
