...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    // 对于端到端模型，输出是 1x300x6 的格式
    // 每行格式为 [x1, y1, x2, y2, conf, class_id]
    float *output_data = (float *)_outputs[0].buf;  // 只有一个输出
    int total_detections = app_ctx->output_attrs[0].dims[1];  // 300
    int num_params = app_ctx->output_attrs[0].dims[2];  // 6

    // 遍历所有检测结果
    for (int i = 0; i < total_detections; i++)
    {
        float conf = output_data[i * num_params + 4];  // 第5个元素是置信度
        int class_id = (int)output_data[i * num_params + 5];  // 第6个元素是类别ID
        
        // 阈值过滤
        if (conf >= conf_threshold && class_id >= 0)
        {
            float x1 = output_data[i * num_params + 0];  // 第1个元素是x1
            float y1 = output_data[i * num_params + 1];  // 第2个元素是y1
            float x2 = output_data[i * num_params + 2];  // 第3个元素是x2
            float y2 = output_data[i * num_params + 3];  // 第4个元素是y2
            
            filterBoxes.push_back(x1);
            filterBoxes.push_back(y1);
            filterBoxes.push_back(x2);
            filterBoxes.push_back(y2);
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        int n = indexArray[i];  // 获取按置信度排序后的索引
        ...
        int id = classId[n];
        float obj_conf = objProbs[n];  // 使用排序后的置信度值

        // 调整坐标到原始图像尺寸
        od_results->results[last_count].box.left = (int)(clamp(x1 - letter_box->x_pad, 0, model_in_w) / letter_box->scale);
        od_results->results[last_count].box.top = (int)(clamp(y1 - letter_box->y_pad, 0, model_in_h) / letter_box->scale);
        od_results->results[last_count].box.right = (int)(clamp(x2 - letter_box->x_pad, 0, model_in_w) / letter_box->scale);
        od_results->results[last_count].box.bottom = (int)(clamp(y2 - letter_box->y_pad, 0, model_in_h) / letter_box->scale);
